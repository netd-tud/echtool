...
func (s *cryptoStream) handleCrypto(off int64, b []byte, f func([]byte) error) error {
	...
	if off == s.in.start {
		...
		//
		// Discard the chunk even if f fails: its range is already recorded
		// in s.inset, so leaving s.in behind would break the invariant that
		// [s.in.start, s.inset[0].end) is stored in the pipe. The connection
		// is closing after an error here, but it still parses incoming
		// packets until it is draining, and a later CRYPTO frame would make
		// the s.in.read below panic with "invalid read range".
		// [echtools local patch — see README-ECHTOOLS.md]
		err := f(b)
